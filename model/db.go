...
func migrate() {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS model_stats (
			...
			race_success_rate REAL DEFAULT 0,
			race_rank INTEGER DEFAULT 0,
			race_role TEXT DEFAULT 'cold_standby',
			PRIMARY KEY (model, channel_id)
		)`,
		`CREATE INDEX IF NOT EXISTS idx_model_stats_role ON model_stats(race_role)`,
		`CREATE INDEX IF NOT EXISTS idx_model_stats_rank ON model_stats(model, race_rank)`,
